      ...
      {open ? (
        <div className="mt-6 border-t border-white/50 pt-6">
          <div className="mb-4 flex items-center gap-2.5">
            ...
            <p className="font-display text-[0.7rem] font-medium">
              Прогресс к цели · {data.targetScore} баллов
            </p>
            <span className="ml-auto text-xs text-muted-foreground">
              не вся программа, только нужное
            </span>
          </div>

          <div
            className={`grid gap-6 ${data.goalPrototypes.length > 0 ? "sm:grid-cols-2" : "grid-cols-1"}`}
          >
            <TopicColumn
              title={`Темы для ${data.targetScore} баллов`}
              icon={<BookOpen className="h-4 w-4 text-primary" />}
              items={data.goalTopics}
            />
            {data.goalPrototypes.length > 0 ? (
              <TopicColumn
                title={`Прототипы для ${data.targetScore} баллов`}
                icon={<Layers className="h-4 w-4 text-primary" />}
                items={data.goalPrototypes}
              />
            ) : null}
          </div>
        </div>
...
export function ExamRadar({ defaultOpen = false }: { defaultOpen?: boolean }) {
  return (
    <>
      {examRadars.map((r) => (
        <RadarCard key={r.id} data={r} defaultOpen={defaultOpen} />
      ))}
    </>
  );
}
