...
const GROUPS_COLLECTION_GROUP = "groups"
...
// already uses.
async function ensureUpcomingDraftsForAllGroups() {
  const snapshot = await db.collectionGroup(GROUPS_COLLECTION_GROUP).get()

  for (const doc of snapshot.docs) {
    const group = doc.data()
    if (!group.teacherId || normalizeScheduleSlots(group).length === 0) {
      continue
    }
...
async function dailyReminderMidday() {
  ...
  await ensureUpcomingDraftsForAllGroups()
